...
void* buscar_dd(t_dd* dd, int chave){
    ...
    // O contador de comparações agora entra como o 3º argumento
    t_elem_dd* e = buscar_lse(dd->entradas[k], &chave, &(dd->comparacoes));
...
void* remover_dd(t_dd* dd, int chave){
    ...
    t_elem_dd* e = remover_conteudo_lse(dd->entradas[k],&chave, &(dd -> comparacoes));
    void* retorno =  NULL;
    if (e!=NULL){
